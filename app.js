...
const searchInput = document.querySelector('.search input'); // 검색 입력 창
...
function startLenderMap(lat, lng) {
  ...
  currentData.forEach((item) => {
    let lating = new naver.maps.LatLng(item.위도, item.경도);
    let bounds = map.getBounds();

    if (bounds.hasLatLng(lating)) {
      //현재 위치를 기준으로 화면 안에 있는지 확인, 화면 내부의 마커만 생성
      var marker = new naver.maps.Marker({
        position: lating,
        map: map,
          ...
          <div class="detail_text">
            ...
            <div class="homepage">
              ...
              <div class="homepage_contents">
                <p><a href="https://www.abc.com">${homePage}</a></p>
              </div>
            </div>
          </div>
    `;
